...
knn1:
**survival_fraction** should get a value above 0.5 if we are interested in the best cases. For the whole dataset, the best cases were on average below 0.5
**random_interleave_fraction** should be low and have a value between 0.05 and 0.5 according to the complete dataset.
...
**budget_log_step** produces good performances for values between -0.5 and 1 but has not a big impact in general. **random_interleave_fraction**should be between 0.25 and 0.75 according to the full dataset. In the subset it doesn't matter. 
...
**budget_log_step** it is hard to tell because of fluctuation but should be at least over -1.5.
...
**survival_fraction** should be under 0.75.
...
It can be seen that there are too many observations to see much. The PCP makes more sense with fewer observations.
After dividing the data, we first look for structural changes.
...
It looks like there a specific ranges for **budget_log_step** which brings better results. 
